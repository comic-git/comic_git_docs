...
Go to [comic\_git\_engine's templates folder](https://github.com/comic-git/comic_git_engine/tree/latest/templates) and find the file you want to change. For comic pages it's `comic.tpl`, for the archive page it's `archive.tpl`, and so on. Open the dropdown for the full list of templates if you aren't sure which one you need.

<details>

<summary>Available templates</summary>

* [404.tpl](https://github.com/comic-git/comic_git_engine/blob/latest/templates/404.tpl) is the layout of the "404 Not Found" page that appears if someone tries to access a page on your site that doesn't exist.
* [archive.tpl](https://github.com/comic-git/comic_git_engine/blob/latest/templates/archive.tpl) is the layout of the Archives page.
* [base.tpl](https://github.com/comic-git/comic_git_engine/blob/latest/templates/base.tpl) is the base template which all the other templates use. If something you want to change is present across the entire site, it's probably in here.
* [comic.tpl](https://github.com/comic-git/comic_git_engine/blob/latest/templates/comic.tpl) is the layout of the individual comic pages.
* [index.tpl](https://github.com/comic-git/comic_git_engine/blob/latest/templates/index.tpl) is the layout of the home page. Edit this file for a more powerful way to update your home page than with `home page.txt`, including being able to use Jinja variables. See also [Template and Hook Data](../expert-editing/template-and-hook-data.md).
* [infinite\_scroll.tpl](https://github.com/comic-git/comic_git_engine/blob/latest/templates/infinite_scroll.tpl) is the layout for the infinite scroll comics page.
* [latest.tpl](https://github.com/comic-git/comic_git_engine/blob/latest/templates/latest.tpl) is the layout for the latest comic page. This differs slightly from the individual comic pages in that comic\_git will always update this one to show the most recent (by post date) comic.
* [md\_page.tpl](https://github.com/comic-git/comic_git_engine/blob/latest/templates/md_page.tpl) is the layout for any .md (Markdown) files present on the site.
* [tagged.tpl](https://github.com/comic-git/comic_git_engine/blob/latest/templates/tagged.tpl) is the layout of the page that displays all comic pages a clicked character or tag appears in.
* [webring.tpl](https://github.com/comic-git/comic_git_engine/blob/latest/templates/webring.tpl) is the layout of the [Webring](../expert-editing/webring.md) block, including both the Previous / Home / Next version and the full member list version.
...
{% hint style="warning" %}
A copied template is a full replacement and does not automatically inherit later engine template fixes. Comic pages use structured `page` and `images` data and positional image anchors. Start from the current default template or update an existing template using [Template and Hook Data](../expert-editing/template-and-hook-data.md).
{% endhint %}
...
{% hint style="info" %}
Editing `comic.tpl` can be intimidating because it contains conditions for many comic_git features. [comic_git_engine's minimal comic template](https://github.com/comic-git/comic_git_engine/blob/latest/extras/comic_minimal.tpl) is a shorter starting point for a custom comic page. Copy it to `/your_content/themes/default/templates/comic.tpl`, then add the features you want and rebuild after each change.
{% endhint %}
