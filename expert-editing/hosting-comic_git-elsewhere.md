...
2. From the base repo directory, run `python comic_git_engine\src\build\build_site.py --delete-scheduled-posts`.
...
In practice, you should treat `OUTPUT_DIR` as required for Neocities deploys. comic\_git will build the finished site into that folder, and the Neocities deployment step will upload that folder.
...
| `OUTPUT_DIR`                | Yes       | The output folder where comic\_git will store the published files. For Neocities deploys, set this explicitly, such as `build`.          |
